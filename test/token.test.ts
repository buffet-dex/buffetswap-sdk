...
describe('Token', () => {
  ...
  describe('#equals', () => {
    it('fails if address differs', () => {
      expect(new Token(ChainId.AVALANCHE, ADDRESS_ONE, 18).equals(new Token(ChainId.AVALANCHE, ADDRESS_TWO, 18))).toBe(
        false
      )
    })
    ...
    it('true on reference equality', () => {
      const token = new Token(ChainId.AVALANCHE, ADDRESS_ONE, 18)
      expect(token.equals(token)).toBe(true)
    })

    it('true even if name/symbol/decimals/projectLink differ', () => {
      const tokenA = new Token(ChainId.AVALANCHE, ADDRESS_ONE, 9, 'abc', 'def', 'https://aaa')
      const tokenB = new Token(ChainId.AVALANCHE, ADDRESS_ONE, 18, 'ghi', 'jkl', 'https://bbb')
